...
calculate_nb_of_neighbors <- function(df, N_neighbors_thres = 1){
  ...
  if (nrow(df) > 1){
    ...
    apply(tmp, MARGIN = 1,
          function(x) sum(x[df$Read.count > N_neighbors_thres] <= 1,
                          na.rm = T)) - 0 # я не поняла зачем это действие нужно, но оставила на всякий случай
  } else {1}
}
...
filter_data_by_nb_of_neighbors <- function(df, N_neighbors_thres = 1){
  ...
  df[, leftgr := .GRP, .(substr(cdr3aa, 1, floor(nchar(cdr3aa) / 2 )),
                         bestVGene, bestJGene)]
  df[, rightgr := .GRP, .(substr(cdr3aa, floor(nchar(cdr3aa) / 2) + 1,
                                 nchar(cdr3aa)), bestVGene, bestJGene)]

  # TODO: тут так-то отдельный трешхолд, надо разобраться с ними

  df[, D_left := calculate_nb_of_neighbors(.SD, nei_read_thres = N_neighbors_thres),
     .(leftgr)]
  df[, D_right := calculate_nb_of_neighbors(.SD, nei_read_thres = N_neighbors_thres),
     .(rightgr)]
  df[, D_id := .N, .(cdr3aa)]
  df[, D := (D_left + D_right - D_id - 1), ]
  # мое творчество вмеcтно кучи NULL
...
all_other_variants_one_mismatch_regexp <- function(str){
  ...
  unique(as.vector(sapply(2 : (nchar(str) -1 ),
                          function(x){tmp <- str; substr(tmp,x,x) <- "X"; tmp})))
}
...
olga_parallel_wrapper_beta <- function(df, cores = 1, chain = "mouseTRB",
  ...
  if (!("ind" %in% colnames(df)))df[, ind :=1:.N, ] #add ind column for sequence combining
  ...
  olga_commands <- paste0("olga-compute_pgen --", chain, " --display_off
                          --time_updates_off	--seq_in 0 --v_in 1 --j_in 2
                          --lines_to_skip 1 -d 'tab' -i tmp", 1:cores,
                          ".tsv -o tmp_out", 1:cores, ".tsv")
  if (withoutVJ) {
    olga_commands<-paste0("olga-compute_pgen --", chain, " --display_off
                          --time_updates_off	 --seq_in 0  --lines_to_skip 1 -d
                          'tab' -i tmp", 1:cores, ".tsv -o tmp_out", 1:cores,
                          ".tsv")
  }
...
pipeline_OLGA <- function(df, Q = 6.27, cores = 1, prompt = F, Read_thres = 0,
  ...
  colnames(df) <- c('Read.count','freq','cdr3nt', 'cdr3aa','bestVGene','d',
  ...
  df <- df[D >= Nb_of_neigbors_thres][, ind := 1:.N, ]

  df_with_mismatch <- df[, .(bestVGene, bestJGene,
              cdr3aa = all_other_variants_one_mismatch_regexp(cdr3aa)), ind]
